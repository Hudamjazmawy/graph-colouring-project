...
void RemoveColor(int node, int color) {
	 ...
	 */
	int node1;
	ColorClass[node] = 0; // ColorClass[node] -> color that is assigned to vertex node, 0 means uncolored
	for(node1 = 0; node1 < num_node; node1++){
		if(node == node1) continue;
		if(adj[node][node1]){
			/*
			...
			if(ColorAdj[node1][color] == 0)
				/*
				 * if currently vertex node1 is adjacent to any colored vertex with color color,
				 * then node1's saturation degree decreased by 1
				 */
				ColorCount[node1]--;
			if(ColorAdj[node1][color] < 0)
				printf("ERROR on assign\n");
			ColorAdj[node1][0]++; // vertex node1's degree in uncolored subgraph increased by 1
		}
	}
}
...
int color(int i, int current_color) {
	 ...
	 */
    	...
    	prob_count++;
    	if(current_color >= BestColoring) return(current_color); // return
    	if(BestColoring <= lb) return(BestColoring); // current_color < BestColoring <= lb, return optimal result 
	if(prob_count >= 100000) return -1;// exceed limit, return
...
int main(int argc, char** argv) {
	...
	if(argc != 3){
		cerr<<argv[0]<<" testFlag <dimacs_file>"<<endl;
		return 1;
	}
	flag = atoi(argv[1]);
	if(flag != 1){
	...
	read_graph_DIMACS_ascii(argv[2]); // read graph from .col file
	prob_count = 0;
	for(i = 0; i < num_node; i++)
		for(j = 0; j < num_node; j++)
			ColorAdj[i][j] = 0;
	...
	if(val == -1)
		...
		if(lbFlag)
			printf("bestColoring %d lb %d subproblems %d\n", val, lb, prob_count);
		else
